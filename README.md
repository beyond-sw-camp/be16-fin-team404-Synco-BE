...
| 권수연 | 두 달 동안 기획부터 개발, 테스트, 배포, 모니터링까지 전체 프로세스를 직접 경험하면서 팀원들과 지속적으로 논의하고 조율하는 과정이 많았다. 이를 통해 단순히 기능을 구현하는 것이 아니라, 서로의 의견을 이해하고 합의점을 찾아가는 의사소통 능력을 크게 키울 수 있었다.<br><br>Synco는 기존의 프로젝트 관리 툴이 갖고 있던 단순한 Task 관리 기능에서 확장하여, 대시보드, 화상회의, 실시간 문서 편집, 검색 고도화와 같은 협업 중심 기능을 추가했다는 점에서 의미가 컸다. 단순히 비슷한 서비스를 따라 만든 것이 아니라, 우리만의 문제 정의와 차별화된 아이디어를 녹여냈다는 점이 특히 뿌듯했다.<br><br>또한 이번 프로젝트에서 MSA 기반의 구조 설계를 처음 시도해보면서, 개발과 기획이 완전히 분리된 것이 아니라 서로 긴밀하게 연결되어 있다는 점을 체감했다. 요구사항이나 ERD를 작성할 때 특정 기능의 변경이 다른 모듈에 어떤 영향을 줄 수 있는지 끊임없이 고민해야 했고, 그 과정에서 시스템을 구조적으로 바라보고 판단하는 사고력을 기를 수 있었다.<br><br>프로젝트 전반에 깊이 관여하면서 책임감도 생겼고, 애정이 가는 결과물을 만들어낼 수 있어 매우 뜻깊은 경험이었다. |
| 김찬진 | 이번 프로젝트를 통해 MSA 아키텍처와 Kafka 기반 이벤트 흐름, 그리고 Elasticsearch 인덱싱까지 직접 설계·구현하면서 백엔드 역량을 확장할 수 있었습니다. 단순히 검색 기능을 붙이는 데 그치지 않고, 도메인 이벤트를 어떻게 끊김 없이 전달할지, 인덱스를 어떻게 설계해야 분석기와 매핑이 자연스럽게 맞을지 계속 분석하고 개선하는 과정에서 많은 배움을 얻었습니다.<br><br>무엇보다 초반 기획을 꼼꼼히 준비해 둔 덕분에 개발이 안정적으로 흘렀습니다. 서비스 모듈 구조부터 ERD, 요구사항 정의서까지 선제적으로 정리해 놓으니 중간에 큰 수정 없이 각자 구현에 집중할 수 있었고, 덕분에 전체 완성도도 한층 높아졌습니다.<br><br>아쉬운 점은 시간 부족으로 Elasticsearch 설계를 충분히 다듬지 못한 것입니다. Spring Data Elasticsearch의 기본 메서드와 간단한 @Query에 머물러 있어, 검색 랭킹 튜닝이나 DSL 최적화를 다음엔 꼭 보완하고 싶습니다. |
